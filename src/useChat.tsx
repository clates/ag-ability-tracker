...
const IsAfterTimestampedLine = (timestampedLine: string, incomingLine: string): boolean => {
...
const getLatestLine = (lines: string[]): string =>
  lines.filter(line => timeStampPattern.test(line)).sort().slice(-1)[0]
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setChatInterval(setInterval(() => {
        ...
        if (newLines.length > 0 && newLines.some(newLine => !lines.has(newLine.text))) {
